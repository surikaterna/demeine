    ...
    if (!isNewStream && self._concurrencyStrategy) {
      ...
      if (nextStreamVersion > aggregate._version) {
        ...
        if (writeOnly) {
          ...
          self._partition.openStream(aggregate.id, false).then(function (stream) {
            shouldThrow = self._concurrencyStrategy(uncommittedEvents, stream.getCommittedEvents());
            resolve(shouldThrow);
          });
        }
      } else {
      ...
      .then(function (uncommittedEvents) {
        ...
        return self.checkConcurrencyStrategy(aggregate, stream, uncommittedEvents).then(function (shouldThrow) {
          ...
          return stream.commit(savingWithId).then(function () {
            ...
            LOG.debug('Unable to save commmit id: ' + savingWithId + ' for type: ' + self.aggregateType + ', with ' + uncommittedEvents.length + ' events.', e);
            throw e;
          });
        });
      });

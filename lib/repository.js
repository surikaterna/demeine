...
Repository.prototype.save = function (aggregate, commitId, callback) {
  ...
  var self = this;
  // if two arguments are provided to the _concurrencyStrategy function need to get all the commits.
      ...
      .then(function (events) {
        ...
        if (!isNewStream && self._concurrencyStrategy) {
          ...
          if (nextStreamVersion > aggregate._version) {
            ...
            var shouldThrow;
            if (writeOnly) {
              shouldThrow = self._concurrencyStrategy(events);
            } else {
              shouldThrow = self._concurrencyStrategy(events, stream.getCommittedEvents());
            }
            if (shouldThrow === true) {

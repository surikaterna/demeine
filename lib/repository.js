...
var Repository = function (partition, aggregateType, factory, concurrencyStrategy, options) {
  ...
  this._optimizeWrites = ('optimizeWrites' in opts) ? options.optimizeWrites : true; // leaving this option true will not read the entire stream, at the cost of not being able to supply all committed events to the concurrencyStrategy.
};
...
Repository.prototype.save = function (aggregate, commitId, callback) {
  ...
  var self = this;
  var writeOnly = self._optimizeWrites;
  return this._partition.openStream(aggregate.id, writeOnly).then(function (stream) {
    return aggregate
      ...
      .then(function (events) {
        ...
        if (!isNewStream && self._concurrencyStrategy) {
          ...
          if (nextStreamVersion > aggregate._version) {
            ...
            var shouldThrow = self._concurrencyStrategy(events, stream.getCommittedEvents()); // stream.getCommittedEvents() will be empty if optimizeWrites is enabled-
            if (shouldThrow === true) {

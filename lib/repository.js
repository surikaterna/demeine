...
Repository.prototype.save = function (aggregate, commitId, callback) {
  ...
  return this._partition.openStream(aggregate.id).then(function (stream) {
    ...
    return stream.commit(savingWithId).then(function () {
      LOG.info('%s committed %d events with %s id', this.aggregateType, events.length, commitId);
      return aggregate;
    }).error(function (e) {
      LOG.error('%s unable to commit %d events with %s id', this.aggregateType, events.length, commitId, e);
      throw e;
    });
  }).nodeify(callback);
};

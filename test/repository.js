...
describe('Repository', function () {
  ...
  describe('#findById', function () {
    ...
    it('hydrates aggregates with snapshot', function (done) {
      var factory = function () { return new Location(); };
      var repo = new Repository(new SnapshotPartition({ id: '1', version: 1, snapshot: { name: 'hello' } }, [{ id: 1, type: 'location.registered_name.event', payload: 'Hello' }]), 'location', factory);
    ...
    it('hydrates aggregates with snapshot and events', function (done) {
      var factory = function () { return new Location(); };
      var repo = new Repository(new SnapshotPartition({ id: '1', version: 1, snapshot: { name: 'hello' } }, [{ id: 1, aggregateId: '1', type: 'location.registered_name.event', payload: 'Hello' }, { id: 2, aggregateId: '1', type: 'location.changed_name.event', payload: 'Hello, world' }]), 'location', factory);
      repo.findById('1').then(function (aggregate) {
        if (aggregate instanceof Location) {
  ...
  it('removes and retries snapshot create when snapshot is broken', function (done) {
    var factory = function () { return new Location(); };
    var repo = new Repository(new SnapshotPartition({ id: '1', version: 1, snapshot: { no_name: 'hello' } }, [{ id: 1, aggregateId: '1', type: 'location.registered_name.event', payload: 'Hello' }, { id: 1, aggregateId: '1', type: 'location.changed_name.event', payload: 'Hello' }, { id: 2, aggregateId: '1', type: 'location.changed_name.event', payload: 'Hello, world' }]), 'location', factory);
    repo.findById('1').then(function (aggregate) {
      if (aggregate instanceof Location) {
        aggregate._getSnapshot().name.should.equal('Hello, world');
        aggregate.getVersion().should.equal(2);
